"""Stage 1 (Groq variant): extract per-clip signal vectors for all 120 wavs.

Mirror of extract_signals.py but with the Groq Whisper-large-v3 ASR backend.
Throttled to 2 parallel workers to stay under the free-tier 30 RPM cap.
...
Writes:
  audit/signal_vectors_groq.json — list of 120 dicts.
...
import lib_audio
import lib_asr_groq as lib_asr  # ← Groq backend
import lib_mos
...
OUT = AUDIT / "signal_vectors_groq.json"

MODELS = ["kokoro", "indic_parler", "indicf5", "springlab_f5"]

# Free-tier 30 RPM cap: 2 parallel workers + 429 retry/backoff keeps us safe.
ASR_PARALLEL = 2
...
def extract_one(model: str, wav_path: Path, gt: dict) -> dict:
    ...
    return {
        ...
        **asr,
        **mos,
        "_asr_backend": "groq-whisper-large-v3",
    }
...
def main() -> int:
    ...
    total = len(jobs)
    print(f"=== Extracting signals for {total} clips with {ASR_PARALLEL} parallel ASR workers (Groq Whisper-large-v3) ===\n")
    print("Free-tier limits: 30 RPM, 2000 RPD, 7200 audio-sec/hr. Total est: ~12 min wall.\n")
    out = [None] * total
    ...
    print("\n=== Per-model summary (Groq Whisper-large-v3) ===")
    for model in MODELS:
