...
def _get_xlit():
    """Return a singleton XlitEngine (Roman→Devanagari, rescore disabled).

    rescore=False skips the 1.5 GB word_prob_dicts load. On the eval-set-tuned
    test words the top transliteration is identical with rescore=True, so this
    only affects RAM, not output quality.
    """
    global _xlit_engine
    if _xlit_engine is None:
        from ai4bharat.transliteration import XlitEngine
        _xlit_engine = XlitEngine(beam_width=4, src_script_type="roman", rescore=False)
    return _xlit_engine
...
def _normalize_ascii_token(tok: str) -> str:
    ...
    IndicXlit's translit_word(word, lang_code) returns a list of beam-search
    candidates ordered by score; we take the top-1.
    """
    key = tok.lower()
    if key in CANONICAL:
    ...
    xlit = _get_xlit()
    try:
        cands = xlit.translit_word(key, "hi")
    except Exception:
        return tok  # last resort: keep original
    if isinstance(cands, list) and cands:
        return cands[0]
    if isinstance(cands, dict):
        # Some package versions return {"hi": [...]} — handle both shapes.
        v = cands.get("hi") or next(iter(cands.values()), [])
        if v:
            return v[0]
    return tok
...
# --- WER / CER metrics (called after both ref and hyp pass through to_unified_devanagari) ---

def compute_cer(reference: str, hypothesis: str) -> float:
    """Character-level edit distance / len(reference). Uses python-Levenshtein.

    Empty-reference convention matches v1.0 (lib_asr.cer): CER=0 if hypothesis
    also empty, else CER=1.
    """
    import Levenshtein
    if not reference:
...
def compute_wer(reference: str, hypothesis: str) -> float:
    """Word-level edit distance via jiwer.

    Empty-reference convention: WER=0 if hypothesis also empty, else WER=1.
    """
    import jiwer
